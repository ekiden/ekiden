...
#[macro_use] extern crate learner_api;
...
fn main() {
    ...
    let data_output = std::process::Command::new("python2")
        ...
        .args(&["--api-proto", &(contract_dir + "/api/src/generated/api_pb2.py")])
        .output()
        .expect("Could not fetch data.");
    assert!(data_output.status.success(), "{}",
        String::from_utf8(data_output.stderr)
            .unwrap_or("Could not generate data".to_string()));


    let examples_proto: Examples = protobuf::parse_from_bytes(&data_output.stdout)
    ...
    let (state, _create_res) = client.create({
        let mut req = CreateRequest::new();
        req.set_requester(user.clone());
        req
    }).expect("error: create");

    let (state, _train_res) = client.train(state, {
    ...
    let infer_res = client.infer(state, {
    ...
    let ground_truth: Vector<f64> = examples.iter().filter_map(|example| {
        unpack_val!(&example.get_features().feature, next_temp)
    }).collect();

    let preds: Vector<f64> = infer_res.get_predictions().iter().filter_map(|example| {
        unpack_val!(&example.get_features().feature, next_temp)
    }).collect();

    assert!(preds.size() == ground_truth.size());

    println!("Training loss: {:?}", (preds - ground_truth).norm(Euclidean));
}
