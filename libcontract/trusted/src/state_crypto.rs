...
use libcontract_common::api::CiphertextAndNonce;
...
pub fn decrypt_state<S: protobuf::MessageStatic>(encrypted_state: &CiphertextAndNonce) -> Result<S, protobuf::ProtobufError> {
    ...
    sodalite::secretbox_open(&mut state_raw_padded, encrypted_state_ciphertext, &encrypted_state_nonce, &state_key).unwrap();
    Ok(protobuf::parse_from_bytes(&state_raw_padded[SECRETBOX_ZEROBYTES..])?)
}

pub fn encrypt_state<S: protobuf::Message>(state: &S) -> Result<CiphertextAndNonce, protobuf::ProtobufError> {
    ...
    // TODO: propagate errors from here?
    sodalite::secretbox(&mut encrypted_state_ciphertext, &state_raw_padded, &encrypted_state_nonce, &state_key).unwrap();
    let mut encrypted_state = CiphertextAndNonce::new();
