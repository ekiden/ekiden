...
impl Tendermint {
    ...
    pub fn broadcast_tx_commit(
        &mut self,
        payload: Vec<u8>,
    ) -> Result<JsonRpcResult<BroadcastTxCommit>, Error> {
        ...
        let uri = String::new() + &self.uri_prefix + "/broadcast_tx_commit?tx=0x" + &payload_hex;
        let resp_str = self.helper(uri)?;
    ...
    pub fn broadcast_tx_async(
        &mut self,
        payload: Vec<u8>,
    ) -> Result<JsonRpcResult<BroadcastTx>, Error> {
        let payload_str = String::from_utf8(payload)?;
        let uri =
            String::new() + &self.uri_prefix + "/broadcast_tx_async?tx=\"" + &payload_str + "\"";
        let resp_str = self.helper(uri)?;
    ...
    pub fn broadcast_tx_sync(
        ...
        let payload_str = String::from_utf8(payload)?;
        let uri =
            String::new() + &self.uri_prefix + "/broadcast_tx_sync?tx=\"" + &payload_str + "\"";
        let resp_str = self.helper(uri)?;
        let result: JsonRpcResult<BroadcastTx> = serde_json::from_str(&resp_str)?;
        Ok(result)
    }
    ...
    pub fn commit(&mut self, height: u64) -> Result<serde_json::Value, Error> {
        ...
        let uri = String::new() + &self.uri_prefix + "/commit?height=" + &height_str;
        let resp_str = self.helper(uri)?;
        let result: serde_json::Value = serde_json::from_str(&resp_str)?;
        Ok(result)
    }
